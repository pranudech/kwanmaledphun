...
const Uploader = ({ setImageFile, imageUrl, showImage = true, showInput = false, onDelete = () => { } }) => {
    ...
    <div key={file.name}>
      <div className="px-6 pt-5 pb-6 border-2 border-dashed rounded-md cursor-pointer relative">
        ...
        <div onClick={() => {
          setFiles([])
          onDelete()
        }} className="absolute top-3 right-3"><FiXCircle size={30} className="text-gray-500" /></div>
      </div>
    </div>
  ));
  ...
  return (
    <div className="w-full text-center">
      {typeof imageUrl === "array" || imageUrl === null ? (
        <>
          ...
          {imageUrl && showImage && (
            <>
              <div onClick={() => {
                setImageFile([])
                onDelete()
              }} className="absolute top-3 right-3"><FiXCircle size={30} className="text-gray-500 absolute top-3 right-3 cursor-pointer hover:text-gray-700" /></div>
              <img
                className="inline-flex max-w-[350px]"
                src={imageUrl}
                alt="product"
